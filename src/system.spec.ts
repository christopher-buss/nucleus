...
import type { StorageObject } from "./system";
...
// eslint-disable-next-line ts/no-unnecessary-type-assertion -- Required to make mutable.
...
export = (): void => {
	...
	describe("A system should", () => {
		...
		it("be able to configure queries", () => {
			const system = new MockSystem();
			let query;
			system.configureQueries = (testWorld: World): void => {
				query = new Query(testWorld).mask;
			};
		...
		it("allow non-default execution groups", () => {
			const temporaryBindableEvent = new Instance("BindableEvent");

			let callCount = 0;

			const system = new MockSystem();
			system.executionGroup = temporaryBindableEvent.Event;
			system.onUpdate = (): void => {
				callCount += 1;
			};
		...
		it("be able to stop all systems", () => {
			...
			system2.executionGroup = temporaryBindableEvent.Event;
		...
		it("be ordered by execution group", () => {
			...
			const event = temporaryBindableEvent.Event;
			...
			temporaryBindableEvent.Destroy();
		});

		itFOCUS("be able to call after systems in order", () => {
		...
		it("be able to use storages", () => {
			const temporaryBindableEvent = new Instance("BindableEvent");
			const event = bindEvent(temporaryBindableEvent.Event);
