...
// eslint-disable-next-line ts/no-unused-vars -- Required for documentation.
import type { ANY, NOT, RawQuery } from "./query";
import { ALL, Query } from "./query";
import type { ExecutionGroup, System, SystemConstructor } from "./system";
...
export class World {
	...
	private readonly componentsToUpdate: SparseSet = new SparseSet();
	private readonly entityManager: EntityManager = new EntityManager();
	/** A set of any component with a registered observer. */
	private readonly observers = new Map<AllComponentTypes, Observer>();
	...
	public destroy(): void {
		...
		for (const index of $range(0, entities.size() - 1)) {
			if (entities[index] !== undefined) {
				this.entityManager.removeEntity(index);
			}
		}
	...
	public hasComponent(entityId: EntityId, component: AnyComponent): boolean {
		const componentId = (component as Internal<AnyComponent>).componentId;
