...
export class EntityManager {
	private readonly empty: Archetype = new Archetype([]);
	/** Entities that are pending removal. */
	private readonly entitiesToDestroy: SparseSet = new SparseSet();
	/** The amount of entities currently alive in the world. */
	private size = 0;

	public archetypes = new Map<string, Archetype>();
	/**
	...
	public alive(entityId: EntityId): boolean {
		// TODO: This should throw an error in dev mode if the entity does not exist.
		return this.entities[entityId]?.has(entityId) ?? false;
	}
