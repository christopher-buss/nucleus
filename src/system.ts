...
import { BitUtils } from "util/bitwise";
import type { ConnectionLike, SignalLike } from "util/connection";
import { ConnectionUtil } from "util/connection";
...
export class SystemManager {
	private readonly executionDefault: ExecutionGroup;
	private readonly executionGroupSignals = new Map<ExecutionGroup, ConnectionLike>();
	private readonly executionGroups = new Set<ExecutionGroup>();
	// private systemArgs?: Array<unknown>;
	private readonly nameToSystem = new Map<SystemName, System>();
	private readonly systemsByExecutionGroup = new Map<ExecutionGroup, Array<System>>();
	 ...
	 * later point.
	 *
	 * @param ctor The constructor of the system that should be disabled.
	 */
	 ...
	 */
	public async scheduleSystem(system: System): Promise<void> {
		...
		debug.profilebegin("SystemManager:scheduleSystems");
		{
			for (const system of systems) {
				...
				if (system.executionGroup !== undefined) {
					...
					// We set the execution group on the system to the actual
					// execution group that it will be executed on. This is
					// required as two instances, e.g. RunService.PreSimulation,
	 ...
	 *
	 * @param _args The arguments to pass to all systems.
	 */
	...
	private ensure(system: SystemInternal, callback: () => void): void {
		...
		if (!success) {
			...
			// TODO: Optionally log to a custom sink.
	...
	private getSystemDependencies(systems: Array<System>): Array<Array<number>> {
		...
		for (const index of $range(0, systems.size() - 1)) {
			masks[index] = new Array<number>(math.ceil(systems.size() / 32), 0);
		}
		...
		for (const index of $range(0, systems.size() - 1)) {
			...
			if (system.after !== undefined) {
				for (const dependency of system.after) {
					...
					const dependencyIndex = systems.findIndex(sys => sys === dependencyInstance);
					...
					masks[index]![math.floor(index / 32)] |= 1 << dependencyIndex % 32;
				}
			}
			...
			if (system.before !== undefined) {
				for (const dependent of system.before) {
					...
					masks[dependencyIndex]![math.floor(dependencyIndex / 32)] |= 1 << index % 32;
				}
			}
		}
		...
		const mergeDependencies = (mask: Array<number>, index: number): void => {
			for (const bit of BitUtils.bits(mask)) {
				mergeDependencies(deepDependencies[bit]!, bit);
				deepDependencies[index]![math.floor(index / 32)] |=
					deepDependencies[bit]![math.floor(bit / 32)]!;
			}
		};

		for (const [index, mask] of Object.entries(deepDependencies)) {
			mergeDependencies(mask, index);
		}

		for (const index of $range(0, deepDependencies.size() - 1)) {
			for (const index_ of $range(0, deepDependencies[index]!.size() - 1)) {
				assert(
					(deepDependencies[index]![index_]! & (1 << index)) === 0,
	 ...
	 *
	 * @returns The ordered systems by explicit dependencies.
	 */
	private orderSystemsByExecutionGroup(unscheduledSystems: Array<System>): Array<System> {
		...
		const addSystem = (
			...
			for (const bit of BitUtils.bits(value)) {
				addSystem(accumulator, dependencies[bit]!, bit);
			}

			if (!accumulator.includes(index)) {
		...
		const order = dependencies.reduce<Array<number>>((accumulator, value, index) => {
			return addSystem(accumulator, value, index);
		}, []);

		return order.map(index => unscheduledSystems[index]!);
	}
	...
	private runSystems(executionGroup: ExecutionGroup): void {
		for (const system of this.systemsByExecutionGroup.get(executionGroup)!) {
			...
			{
				this.ensure(system, () => {
					// eslint-disable-next-line no-inline-comments -- Temporary
	...
	private sortSystems(systems: Array<System>): void {
		debug.profilebegin("SystemManager:sortSystems");
		{
			...
			for (const [group, systemsInGroup] of this.systemsByExecutionGroup) {
				this.systemsByExecutionGroup.set(
					group,
					this.orderSystemsByExecutionGroup(systemsInGroup),
				);
			}
		}
